...
def new_apple_position(screen_size, apple_size, snake):
    ...
    :param apple_size: [apple_width, apple_height] in pixels.
    ...
    """
    empty_space = True
    x = random.randrange(0, screen_size[0], apple_size[0])
    y = random.randrange(0, screen_size[1], apple_size[1])
...
def update_segment_position(segment, segment_size, screen_size):
    ...
    :param segment_size: [segment_width, segment_height] in pixels.
    :param screen_size: [screen_width, screen_height] in pixels.
    ...
    if segment[2] == 'U':
        ...
        if segment[1] <= -20:
            segment[1] = screen_size[1] - 20
    elif segment[2] == 'D':
        segment[1] += segment_size[1]
        if segment[1] >= screen_size[1]:
            segment[1] = 0
    elif segment[2] == 'L':
        segment[0] -= segment_size[0]
        if segment[0] <= -20:
            segment[0] = screen_size[0] - 20
...
def update_snake_position(snake, segment_size, screen_size):
    ...
    :param snake: a list of sub-lists of length 3, where a sub-list is [x_position, y_position, direction].
    :param segment_size: the width and height of each segment, as a list.
...
def add_new_segment(snake, segment_size, screen_size):
    ...
    :param segment_size: [segment_width, segment_height], where both parts are positive integers.
    :param screen_size: [screen_width, screen_height] in pixels.
...
def play_snake(speed, screen_tuple, clock):
    """
    Runs a playthrough of the retro_games game, and returns the final score when game over occurs.
    ...
    snake = [[200, 240, 'R'], [180, 240, 'R'], [160, 240, 'R'], [140, 240, 'R'], [120, 240, 'R'], [100, 240, 'R']]
    input_direction = 'R'
    score = 0
    apple_size = [segment_size[0], segment_size[1]]
    ...
    while not done:
        ...
        # Draw the apple.
        pygame.draw.rect(screen, red, [apple_position[0], apple_position[1], 19, 19])
